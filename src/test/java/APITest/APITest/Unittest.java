...
public class Unittest {
    ...
    public void f() throws Exception {
        String exePath = "C:\\Users\\ghmd8\\Downloads\\chromedriver_win32\\chromedriver.exe";
        ...
        System.out.println("APITest.APITest.Unittest.f()" + driver.toString());
        WebElement email = driver.findElement(By.id("email"));
        System.out.println("APITest.APITest.Unittest.f()" + email.toString());

        email.sendKeys("ghulam.taher@pressganey.com");
        System.out.println(".........");
        ...
        h = wait.until(ExpectedConditions.visibilityOfElementLocated(By.id("i0118")));
        h.sendKeys("Baseball2009");
        // h = wait.until(ExpectedConditions.visibilityOfElementLocated(By.id("next")));
        h = wait.until(ExpectedConditions.visibilityOfElementLocated(By.id("idSIButton9")));
        h.click();
...
// 	Sytem.out.pri
        ...
        Object xsrfToken = json.get("token");
        System.out.println(xsrfToken);

// to read request
        JSONParser jsonParser = new JSONParser();
        JSONObject usersearRequest = null;
        try (FileReader reader = new FileReader(new File("src/test/java/APITest/APITest/UserSearch.json"))) {
            //Read JSON file
            usersearRequest = (JSONObject) jsonParser.parse(reader);
        ...
        for (org.openqa.selenium.Cookie cookie : allcookies) {
            ...
            if (cookie.getName().equalsIgnoreCase("Identity")) {
                identity = cookie.getValue();
            }
        }
        RequestSpecification request = RestAssured.given().cookies(new Cookies(restAssuredCookies));
        if (identity == null) {
            throw new Exception("No Identity cookie found");
        }
        RequestSpecification cook = RestAssured.given().cookie("Identity", identity);
        RequestSpecification headers = cook.headers("x-xsrf-token", xsrfToken);
        RequestSpecification body = headers.contentType(ContentType.JSON);
        RequestSpecification body1 = body.body(usersearRequest.toJSONString());

        Response post = body1.when().post("/api/uam/users/search");
